...
-- 1) Trigger: modifica cliente con autista → 'da_confermare' (autista mantenuto), senza → 'nuovo'
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Operatore riprende in mano un servizio precedentemente modificato dal cliente
    ...
    IF is_client THEN
      ...
      IF OLD.stato IN ('da_confermare','confermato','in_corso','nuovo') THEN
        -- Se c'è un autista assegnato, il servizio va rivisto ma resta assegnato
        IF has_driver THEN
          NEW.stato := 'da_confermare';
        ELSE
          NEW.stato := 'nuovo';
    ...
    -- Assegnazione autista da dashboard → da_confermare (solo se prima non c'era)
  ...
  IF array_length(v_changed_fields, 1) > 0 THEN
    SELECT string_agg(
      CASE field_name
        ...
        WHEN 'allegato_nome' THEN 'Allegato' ELSE field_name
...
-- 3) Backfill: righe con cliente modificato + autista assegnato ma stato 'nuovo' → 'da_confermare'
UPDATE public.servizi
   SET stato = 'da_confermare'
 WHERE modificato_da_cliente = true
   AND stato = 'nuovo'
   AND (autista_id IS NOT NULL OR autista_esterno_id IS NOT NULL);
