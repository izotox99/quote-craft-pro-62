...
export default function Clients() {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: "", email: "", company: "", phone: "", notes: "" }); setDialogOpen(true); };
  const openEdit = (c: Client) => { setEditing(c); setForm({ name: c.name, email: c.email ?? "", company: c.company ?? "", phone: c.phone ?? "", notes: c.notes ?? "" }); setDialogOpen(true); };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="font-display text-2xl font-bold text-foreground">Clienti</h1>
            <p className="text-sm text-muted-foreground">Gestisci i clienti della tua organizzazione</p>
          </div>
        ...
        <div className="relative max-w-sm">
          ...
          <Input placeholder="Cerca clienti..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="py-16 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">{search ? "Nessun cliente trovato" : "Nessun cliente"}</p>
                {!search && (
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editing ? "Modifica cliente" : "Aggiungi cliente"}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <Label>Nome *</Label>
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nome cliente" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label>Email</Label>
                  <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="email@esempio.com" />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label>Società</Label>
                <Input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} placeholder="Nome società" />
              </div>
              <div className="space-y-2">
                <Label>Note</Label>
              ...
              <Button className="w-full" onClick={handleSave}>
                {editing ? "Aggiorna cliente" : "Aggiungi cliente"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
